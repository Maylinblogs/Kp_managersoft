...
public class ClaimDao implements DAO<Claim> {
    ...
    public boolean saveEntity(Claim entity) {
        ...
        try {
            JDBC.connect();
            PreparedStatement preparedStatement = JDBC.connection
                    .prepareStatement("INSERT INTO claims (id_worker,name_claim,status_claim,info)" +
                            " values (?,?,?,?) ");
    ...
    public boolean updateEntity(Claim entity) {
        ...
        try {
            JDBC.connect();
            PreparedStatement preparedStatement = JDBC.connection
                    .prepareStatement("UPDATE claims SET status_claim =? where claim_id = ?");
            preparedStatement.setInt(1, entity.getStatus_claim());
            preparedStatement.setInt(2, entity.getClaim_id());
            preparedStatement.executeUpdate();
            isTrue = true;
        } catch (SQLException e) {
            throw new RuntimeException(e);
